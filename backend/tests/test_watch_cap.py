...
def _migration_module():
    """Loaded by path: `0026`'s filename is not an importable module name, and
    a migration is deliberately not on the import path."""
    ...
    path = (
        ...
        / "d4a91c7b6e02_0026_cap_active_surfaces.py"
    )
...
def test_the_migration_ranks_the_same_pages_the_app_would():
    """0026 carries a frozen copy of the ranking on purpose, so this pins the
    two together at the point they were written. If they ever diverge, the
    rows the migration deactivated stop matching the ones the running app
    would have chosen — and the disagreement is silent."""
    ...
    migration_order = [r.id for r in sorted(rows, key=migration._rank)]

    assert app_order == migration_order
